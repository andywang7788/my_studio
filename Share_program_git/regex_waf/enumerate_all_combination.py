...
def list_all_combination_in_one_layer(test_string: str, left_boundary: int, right_boundary: int,
    ...
    '''
    sub_string = test_string[left_boundary:right_boundary + 1]

    prefix = my_reg_function.Get_prefix(sub_string)
    print("substring is: %s" % sub_string)
    ...
    for i, v in enumerate(OR_location_list):
        option = ""  # tail string
        # insert def to get the prefix of "(?:" or "("
        ...
        if i > 0:
            ...
            # print(OR_location_list[i - 1]+1,v)
                ...
                # add the prefix in front of string.
            result.append(r"{Prefix}{string1})".format(Prefix=prefix, string1=option))
